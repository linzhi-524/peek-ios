...
```text
peek-ios-server：后端。iOS 快捷指令 / Android App 填这个地址。
peek-ios-mcp：MCP。ChatGPT / MCP 客户端填这个地址 + /mcp。
```
...
```text
https://peek-ios-server-xxxx.onrender.com/health
```
...
打开 `peek-ios-mcp`，复制它的地址，例如：

```text
https://peek-ios-mcp-xxxx.onrender.com
```
...
```text
https://peek-ios-mcp-xxxx.onrender.com/health
```
...
然后把 `peek-ios-server` 的 `LINJIAN_TOKEN` 改成这串，保存后 redeploy。
...
URL：https://peek-ios-mcp-xxxx.onrender.com/mcp
```
...
如果这里有内容，说明后端收到了。再检查 MCP 地址是不是填的 `peek-ios-mcp`，不是 `peek-ios-server`。
