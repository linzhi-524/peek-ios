...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path in ("/api/device/state", "/api/life_state", "/api/ios/state"):
            if not self._require_token(): return
            default_device = "ios-phone" if path == "/api/ios/state" else DEFAULT_DEVICE
            device_id = qs.get("device_id", [default_device])[0] or default_device
    ...
    def do_POST(self) -> None:
        ...
        if path in ("/api/device/state", "/api/ios/state"):
            if not self._require_token(): return
            data = self._read_json()
            default_device = "ios-phone" if path == "/api/ios/state" else DEFAULT_DEVICE
            device_id = data.get("device_id") or default_device
            if path == "/api/ios/state":
                data.setdefault("platform", "ios")
                data.setdefault("life_state_version", "ios-lite-0.1")
                data.setdefault("current_app", data.get("frontmost_app", "iOS / Shortcuts"))
                data.setdefault("current_package", "ios.shortcuts")
                data.setdefault("accessibility_ready", False)
                data.setdefault("usage_permission_ready", False)
        ...
        if path in ("/api/screenshot", "/api/ios/screenshot"):
            if not self._require_token(): return
